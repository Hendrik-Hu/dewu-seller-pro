...
create index if not exists products_user_deleted_idx
...
begin
  ...
  set stock = v_new_stock, status = case when v_new_stock = 0 then 'sold' else status end
  where id = p_product_id and user_id = p_user_id and deleted_at is null;
  ...
  return jsonb_build_object('product_id', p_product_id, 'activity_id', v_activity_id, 'stock', v_new_stock);
end;
$$;
