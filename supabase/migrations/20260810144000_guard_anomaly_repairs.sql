...
create or replace function public.guard_negative_stock_updates()
returns trigger
language plpgsql
...
begin
  if old.stock < 0
    and coalesce(current_setting('app.inventory_anomaly_repair', true), '') <> 'allowed'
  then
    raise exception 'Negative stock records can only be changed through data health repair';
  end if;
  return new;
end;
$$;

drop trigger if exists products_guard_negative_stock_update on public.products;
create trigger products_guard_negative_stock_update
before update on public.products
for each row execute function public.guard_negative_stock_updates();
...
begin
  ...
  return jsonb_build_object(
    'audit_id', v_audit_id, 'target_table', p_target_table,
    'record_id', p_record_id, 'field_name', v_field_name,
...
revoke all on function public.guard_negative_stock_updates() from public, anon, authenticated;
