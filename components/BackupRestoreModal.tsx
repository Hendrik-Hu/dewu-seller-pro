          ...
          <section>
            ...
            <div className="divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-100 bg-white dark:divide-zinc-800 dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <button onClick={exportJson} disabled={Boolean(busy)} className="flex w-full items-center gap-3 p-3 text-left disabled:opacity-50">
                <FileJson size={20} className="text-blue-600" /><div className="flex-1"><div className="text-sm font-medium text-slate-800 dark:text-white">JSON 账本包</div><div className="text-[11px] text-slate-400">商品、流水、费用方案、估算与实际结算审计、仓库及修复记录</div><div className="mt-1 text-[10px] text-amber-600">更新版完整备份请恢复到空账本，不支持跨版本叠加。</div></div>{busy === 'json' ? <Loader2 size={17} className="animate-spin" /> : <Download size={17} />}
              </button>
            </div>
