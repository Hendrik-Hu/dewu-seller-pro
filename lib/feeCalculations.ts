import type { FeeQuote, FeeScheme } from '../types';
...
export const validateFeeScheme = (scheme: Omit<FeeScheme, 'id' | 'updatedAt'>) => {
...
export const calculateSchemeFee = (unitSalePrice: number, quantity: number, scheme: FeeScheme) => {
...
export const calculateBreakEvenUnitPrice = (unitCost: number, quantity: number, scheme: FeeScheme, manualFeeOverride?: number) => {
  if (!Number.isInteger(quantity) || quantity <= 0) throw new Error('数量必须是正整数');
